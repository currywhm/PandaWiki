...
// RAGLiteResponse is the response format from RAGLite API (uses code instead of success)
type RAGLiteResponse struct {
	Code    int             `json:"code"`
	Message string          `json:"message"`
	Data    json.RawMessage `json:"data"`
}

// DatasetResponse is the dataset data structure
...
func (s *CTRAG) CreateKnowledgeBase(ctx context.Context) (string, error) {
	// Use direct HTTP call to handle RAGLite's response format (code instead of success)
	reqBody := map[string]string{"name": uuid.New().String()}
	jsonBody, err := json.Marshal(reqBody)
	if err != nil {
		return "", fmt.Errorf("failed to marshal request: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", s.baseURL+"/api/v1/datasets", bytes.NewReader(jsonBody))
	if err != nil {
	...
	if err := json.Unmarshal(result.Data, &dataset); err != nil {
