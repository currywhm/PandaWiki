...
func (r *KnowledgeBaseRepository) CreateKnowledgeBase(ctx context.Context, maxKB int, kb *domain.KnowledgeBase) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := r.SyncKBAccessSettingsToCaddy(ctx, kbs); err != nil {
			r.logger.Warn("failed to sync kb access settings to caddy (non-fatal, using nginx)", "error", err)
			// Don't return error - allow KB creation to succeed without Caddy
		}
		type AppBtn struct {
			ID       string `json:"id"`
...
func (r *KnowledgeBaseRepository) UpdateKnowledgeBase(ctx context.Context, req *domain.UpdateKnowledgeBaseReq) (bool, error) {
	...
	if err = r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := r.SyncKBAccessSettingsToCaddy(ctx, kbs); err != nil {
			r.logger.Warn("failed to sync kb access settings to caddy (non-fatal)", "error", err)
			// Don't return error - allow operation to succeed without Caddy
		}
		return nil
	}); err != nil {
		return false, err
...
func (r *KnowledgeBaseRepository) DeleteKnowledgeBase(ctx context.Context, kbID string) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := r.SyncKBAccessSettingsToCaddy(ctx, kbs); err != nil {
			r.logger.Warn("failed to sync kb access settings to caddy (non-fatal)", "error", err)
			// Don't return error - allow deletion to succeed without Caddy
		}
		return nil
	})
}
